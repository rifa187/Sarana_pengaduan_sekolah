                        ...
                        <div class="rounded-2xl overflow-hidden border border-slate-200 shadow-sm group relative">
                            <?php if(\Illuminate\Support\Str::endsWith(strtolower($complaint->bukti_file), ['.pdf'])): ?>
                                <div class="p-8 bg-slate-50 flex flex-col items-center justify-center text-center h-48 group-hover:bg-slate-100 transition-colors">
                                    ...
                                    <a href="<?php echo e(\Illuminate\Support\Facades\Storage::url($complaint->bukti_file)); ?>" target="_blank" class="mt-3 text-blue-600 hover:text-blue-800 text-[13px] font-bold">Lihat Dokumen</a>
                                </div>
                            <?php else: ?>
                                <div class="relative block overflow-hidden aspect-[4/3] bg-slate-100">
                                    <img src="<?php echo e(\Illuminate\Support\Facades\Storage::url($complaint->bukti_file)); ?>" alt="Bukti Lampiran" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500">
                                    <div class="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                                        <a href="<?php echo e(\Illuminate\Support\Facades\Storage::url($complaint->bukti_file)); ?>" target="_blank" class="px-5 py-2 bg-white/90 backdrop-blur rounded-full text-sm font-bold text-slate-800 shadow-lg">Perbesar</a>
                                    </div>
                                </div>
                            <?php endif; ?>
                        </div>
                    ...
                    <div class="relative flex items-start gap-6">
                        ...
                        <div class="bg-slate-50/80 border border-slate-100 rounded-2xl p-5 w-full relative">
                            ...
                            <div class="flex justify-between items-center mb-2">
                                ...
                                <span class="text-xs font-semibold text-slate-400 bg-white px-2 py-1 rounded-md border border-slate-100"><?php echo e($response->created_at->diffForHumans()); ?></span>
                            </div>
                            <p class="text-slate-600 leading-relaxed text-[13.5px] mt-2"><?php echo e($response->tanggapan); ?></p>
                        </div>
                    </div>
