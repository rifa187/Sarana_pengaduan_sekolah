                        ...
                        <div class="rounded-xl overflow-hidden border border-slate-200 shadow-sm group relative">
                            @if(\Illuminate\Support\Str::endsWith(strtolower($complaint->bukti_file), ['.pdf']))
                                <div class="p-6 bg-gradient-to-br from-red-50 to-red-100/50 flex flex-col items-center justify-center text-center h-40">
                                    ...
                                    <a href="{{ Storage::url($complaint->bukti_file) }}" target="_blank" class="mt-2 inline-flex items-center gap-1 text-red-600 hover:text-red-700 text-xs font-bold transition-colors">
                            ...
                            @else
                                <div class="relative block overflow-hidden aspect-[4/3] bg-slate-100">
                                    <img src="{{ Storage::url($complaint->bukti_file) }}" 
                                         alt="Bukti Laporan" 
                                         class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300"
                                         onerror="this.style.display='none'; document.getElementById('img-error-{{$complaint->id}}').style.display='flex'">
                                    <div class="absolute inset-0 bg-black/0 group-hover:bg-black/20 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                                        <a href="{{ Storage::url($complaint->bukti_file) }}" target="_blank" class="px-4 py-2 bg-white/95 backdrop-blur rounded-lg text-xs font-bold text-slate-800 shadow-lg hover:bg-white transition-colors">
                                            Perbesar Foto
                                        </a>
                                    </div>
                                </div>
                                <div id="img-error-{{$complaint->id}}" class="hidden w-full aspect-[4/3] bg-slate-50 flex flex-col items-center justify-center text-center">
                ...
                <div class="space-y-4">
                    ...
                    <div class="bg-slate-50 border border-slate-200 rounded-lg p-4">
                        <div class="flex justify-between items-start mb-2">
                            ...
                            <span class="text-xs text-slate-400">{{ $response->created_at->format('d M Y, H:i') }}</span>
                        </div>
                        <p class="text-slate-700 text-sm leading-relaxed">{{ $response->tanggapan }}</p>
                    </div>
                    @endforeach
                </div>
            @else
